In an endless pursuit of knowledge, Scholars tend to be logical and goal oriented. Many Scholars dedicate their lives to academia, fine arts, engineering and other intellectually-driven crafts.

Scholars are innovators, their cleverness and ambition often surpassing contemporary technologies and social norms. They are daring and willing to push beyond the unknown to discover new depths.
## Starting Characteristics
Armour: 0
Wound Capacity: 8
## Archetype Skills
### Blueprints
When making a roll, you can deposit any one of the dice, by excluding it from the current roll, and keeping it as a Blueprint Die. You can later use it in subsequent rolls by withdrawing it from your sheet. You can store up to 3 Blueprint Dice at any given moment.
### Improvised Schematics (depracated)
When crafting items and gadgets, you may substitute materials which you are missing (at the GMs discretion) with a Blueprint Die of a value equal to the item’s value.
### Delayed Epiphany
Once your Blueprint Dice are full, and after making a roll, you may decide to store all of the dice in their current state, failing the roll as a result. At any later point you may add all of those in their current state dice to a different roll. 